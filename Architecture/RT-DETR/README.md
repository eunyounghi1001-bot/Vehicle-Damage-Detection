...
### 1. 연구 배경 (Motivation)
기존의 객체 탐지 시장은 **YOLO**가 지배하고 있었으나, YOLO는 **NMS(Non-Maximum Suppression)**라는 후처리가 필수적이라 최적화가 어렵고 부정확한 경우가 있었습니다. 반면, **DETR**은 NMS가 필요 없는 혁신적인 구조였지만 연산량이 너무 많아 '실시간(Real-time)'으로 쓰기엔 너무 느렸습니다.
...
### 2. RT-DETR의 핵심 아이디어
RT-DETR은 세계 최초로 실시간 추론이 가능한 **End-to-End Object Detector**입니다.
...
| 장점 | 단점 |
| :--- | :--- |
| ✅ **속도:** YOLOv8보다 빠르면서 더 높은 정확도 | ❌ **구조 복잡도:** 구현 및 커스텀 난이도가 높음 |
| ✅ **End-to-End:** NMS 미사용으로 튜닝 단순화 | ❌ **학습 데이터:** 성능 극대화에 대량의 데이터 필요 |
| ✅ **Flexible:** 시나리오별 속도/정확도 조절 가능 | |
